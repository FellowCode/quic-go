...
func toAdaptiveBDPDebugInfo(info congestion.AdaptiveBDPDebugInfo) AdaptiveBDPDebugInfo {
	telemetry := make([]AdaptiveBDPTelemetrySample, len(info.Telemetry))
	for i, sample := range info.Telemetry {
		telemetry[i] = AdaptiveBDPTelemetrySample{
			Event:                           sample.Event,
			Elapsed:                         sample.Elapsed,
			RoundCount:                      sample.RoundCount,
			State:                           sample.State,
			TransitionReason:                sample.TransitionReason,
			CongestionWindow:                uint64(sample.CongestionWindow),
			TargetCwnd:                      uint64(sample.TargetCwnd),
			BytesInFlight:                   uint64(sample.BytesInFlight),
			BDP:                             uint64(sample.BDP),
			BandwidthBytesPerSecond:         sample.BandwidthBytesPerSecond,
			MaxBandwidthBytesPerSecond:      sample.MaxBandwidthBytesPerSecond,
			ShortBandwidthBytesPerSecond:    sample.ShortBandwidthBytesPerSecond,
			RecoveryBandwidthBytesPerSecond: sample.RecoveryBandwidthBytesPerSecond,
			PacingRateBytesPerSecond:        sample.PacingRateBytesPerSecond,
			PacingGain:                      sample.PacingGain,
			CwndGain:                        sample.CwndGain,
			LatestRTT:                       sample.LatestRTT,
			SmoothedRTT:                     sample.SmoothedRTT,
			MinRTT:                          sample.MinRTT,
			QueueDelay:                      sample.QueueDelay,
			SharedQueueDelay:                sample.SharedQueueDelay,
			ControlQueueDelay:               sample.ControlQueueDelay,
			QueueTarget:                     sample.QueueTarget,
			QueueState:                      sample.QueueState,
			LossRatioRound:                  sample.LossRatioRound,
			LossRatioEWMA:                   sample.LossRatioEWMA,
			LostBytesThisRound:              uint64(sample.LostBytesThisRound),
			AckedBytesThisRound:             uint64(sample.AckedBytesThisRound),
			HasRecentECNCE:                  sample.HasRecentECNCE,
			LastLossActionReason:            sample.LastLossActionReason,
			LastLossCwndMultiplier:          sample.LastLossCwndMultiplier,
			LastLossPacingMultiplier:        sample.LastLossPacingMultiplier,
			PacingCutMultiplier:             sample.PacingCutMultiplier,
			PacingCutRemaining:              sample.PacingCutRemaining,
			UploadWarmupActive:              sample.UploadWarmupActive,
			IdleRestartActive:               sample.IdleRestartActive,
			ProbeUpActive:                   sample.ProbeUpActive,
			ProbeDownActive:                 sample.ProbeDownActive,
			ProbeRTTActive:                  sample.ProbeRTTActive,
			FullBwReached:                   sample.FullBwReached,
		}
	}
	return AdaptiveBDPDebugInfo{
		...
		SharedQueueDelay:  info.SharedQueueDelay,
		ControlQueueDelay: info.ControlQueueDelay,
		QueueTarget:       info.QueueTarget,
		QueueState:        info.QueueState,
		PacingGain:        info.PacingGain,
		CwndGain:          info.CwndGain,

		NegativeBandwidthConfidence:    info.NegativeBandwidthConfidence,
		HasCongestionEvidence:          info.HasCongestionEvidence,
		PipeForDownshift:               uint64(info.PipeForDownshift),
		PipeFillThreshold:              uint64(info.PipeFillThreshold),
		ActiveBandwidthBeforeDownshift: info.ActiveBandwidthBeforeDownshift,
		NoCongestionRateFloor:          info.NoCongestionRateFloor,
		NoQueueLowRounds:               info.NoQueueLowRounds,
		NoQueueLowAcked:                uint64(info.NoQueueLowAcked),

		LossRatioRound:               info.LossRatioRound,
		LossRatioEWMA:                info.LossRatioEWMA,
		LostBytesThisRound:           uint64(info.LostBytesThisRound),
		AckedBytesThisRound:          uint64(info.AckedBytesThisRound),
		LossMinBytes:                 uint64(info.LossMinBytes),
		EmergencyLossMinBytes:        uint64(info.EmergencyLossMinBytes),
		MinLossSampleBytes:           uint64(info.MinLossSampleBytes),
		LossGraceRatio:               info.LossGraceRatio,
		LossSevereThreshold:          info.LossSevereThreshold,
		EmergencyLossThreshold:       info.EmergencyLossThreshold,
		QueuePressure:                info.QueuePressure,
		MildLossRounds:               info.MildLossRounds,
		LastLossActionReason:         info.LastLossActionReason,
		LastLossCwndMultiplier:       info.LastLossCwndMultiplier,
		LastLossPacingMultiplier:     info.LastLossPacingMultiplier,
		LastLossCutbackRound:         info.LastLossCutbackRound,
		SuppressProbeUpUntilRound:    info.SuppressProbeUpUntilRound,
		SuppressProbeUpReason:        info.SuppressProbeUpReason,
		LossFreeRounds:               info.LossFreeRounds,
		LastMaterialLossRound:        info.LastMaterialLossRound,
		LossRecoveryProbeActive:      info.LossRecoveryProbeActive,
		LossRecoveryProbeBW:          info.LossRecoveryProbeBW,
		LossRecoveryProbeUntilRound:  info.LossRecoveryProbeUntilRound,
		HasLastECNCE:                 info.HasLastECNCE,
		LastECNCERound:               info.LastECNCERound,
		PersistentCongestionEvents:   info.PersistentCongestionEvents,
		LastPersistentCongestionSpan: info.LastPersistentCongestionSpan,
		LastPersistentCongestionGate: info.LastPersistentCongestionGate,
		MaxOutstandingSentPackets:    uint64(info.MaxOutstandingSentPackets),
		MaxTrackedSentPackets:        uint64(info.MaxTrackedSentPackets),
		TrackedSentPackets:           uint64(info.TrackedSentPackets),
...
func toCongestionCwndTuningConfig(c CwndTuning) congestion.CwndTuningConfig {
	...
	return congestion.CwndTuningConfig{
		Enable:                           c.Enable,
		EnableAdaptiveBDPTelemetry:       c.EnableAdaptiveBDPTelemetry,
		Algorithm:                        algorithm,
		...
		MinProbeRateBps:                  c.MinProbeRateBps,
		...
		LossMinBytes:                     c.LossMinBytes,
		EmergencyLossMinBytes:            c.EmergencyLossMinBytes,
		MinLossSampleBytes:               c.MinLossSampleBytes,
		LossEWMAAlpha:                    c.LossEWMAAlpha,
		MaxLossCwndCutNoQueue:            c.MaxLossCwndCutNoQueue,
		MaxLossCwndCutWithQueue:          c.MaxLossCwndCutWithQueue,
		MinLossCwndCut:                   c.MinLossCwndCut,
		...
		MildLossPersistentRounds:         c.MildLossPersistentRounds,
		LossRecoveryProbeRounds:          c.LossRecoveryProbeRounds,
		LossRecoveryProbeGain:            c.LossRecoveryProbeGain,
		LossRecoveryProbeDurationRounds:  c.LossRecoveryProbeDurationRounds,
		LossRecoveryClearShortBwFraction: c.LossRecoveryClearShortBwFraction,
		...
		UploadWarmupBytes:                c.UploadWarmupBytes,
		MinDownshiftSampleBytes:          c.MinDownshiftSampleBytes,
		CongestionDownshiftRounds:        c.CongestionDownshiftRounds,
